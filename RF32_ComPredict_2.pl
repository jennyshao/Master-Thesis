...
my $sfile1 = "./RF32-filelist";
my $sfile2 = "./RF32_predict.txt";
my $rfile = "./RF32_FinalPrediction.txt";
my $sstring1;
my $sstring2;
my $rstring;
...
while($sstring1=<SFILE1>){
    ...
    $sstring2 =~ s/^.*://;
    $ratio_string = $sstring2;
    $ratio_string =~ s/^\w+\s+//; 
    $sstring2 =~ s/\s+\d+\.\d+|\s+\d//;
    $sstring2 =~ s/\s$//;
    $string = $sstring1."-".$sstring2."#".$ratio_string."\n";
    push @predict, $string;
}
print RFILE @predict;
...
print RFILE "There are ".$sum." cases "."\n";
